/* vikings.js — Vikings Vengeance page toys and easter eggs.
   ...
   deviation egg. Whisper strings are read lazily from the active dictionary
   (English fallback until the i18n phase adds vv.* keys). */
(function () {
  'use strict';
  ...
  function boot(BH) {
    ...
    if (cities && outline) {
      function paint() {
        ...
        if (n === 0) {
          outline.innerHTML = '<b>0</b> garrisons \u2014 just the fixed home wave reward. troops at home earn nothing.';
        } else {
          outline.innerHTML = '<b>' + n + '</b> garrisons score for you at once \u2014 <b>' + n +
            '\u00D7</b> the kill surface of staying home.';
        }
        if (n === 6 && !BH.eggSeen(27)) {
          BH.markEgg(27);
          BH.showNote(BH.tr('vv.egg.toy', 'six garrisons. the bear sails with all of them.'), cities.closest('.calc'));
        }
      }
      cities.addEventListener('input', paint);
      paint();
    }
  }
